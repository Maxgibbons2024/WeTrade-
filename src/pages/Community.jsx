...
import { formatCurrency, formatDate, CLOSERS, MENTORS, PACKAGES } from '../lib/constants';
...
export default function Community() {
  ...
  // ---- Leaderboards ----
  const reviewsByMentor = useMemo(() => {
    const map = {};
    students.filter((s) => s.trustpilot_review).forEach((s) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Trustpilot by Mentor */}
        <div className="bg-[#1a1d20] rounded-xl border border-gray-800 p-5">
          <h3 className="text-sm font-medium text-gray-400 mb-4">Trustpilot Reviews by Mentor</h3>
          {reviewsByMentor.length === 0 ? (
            <p className="text-xs text-gray-600">No reviews yet</p>
          ) : (
            <div className="space-y-3">
              {reviewsByMentor.map(([name, count], i) => (
                <div key={name} className="flex items-center gap-3 p-3 rounded-lg bg-white/[0.02]">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-green-400">{count}</p>
